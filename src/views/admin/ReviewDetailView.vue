...
function riskSuggestion(alert) {
  ...
  }[alert.code] || '请结合当前疑点补充说明或要求供应商整改。'
}
...
const issueCards = computed(() => {
  ...
  ;(record.value.riskAlerts || []).forEach((alert) => {
    cards.push({
      ...
      title: alert.label,
      level:
        alert.severity === 'critical' ? 'danger' : alert.severity === 'high' ? 'danger' : alert.severity === 'medium' ? 'warning' : 'info',
...
const checklist = computed(() => {
  ...
  const entityMismatch = (record.value.comparisons || []).some(
    (item) =>
      ['企业名称', '统一社会信用代码', '法人'].includes(item.field) &&
      comparisonLevel(item.result) === 'mismatch',
  )
  const scopePending = (record.value.comparisons || []).some(
    (item) =>
      ['认证范围', '专利名称'].includes(item.field) &&
      comparisonLevel(item.result) !== 'match',
  )
  ...
  return [
    {
      label: '有效期与基础风险',
      status: renewalLevel === 'expired' ? 'danger' : renewalLevel === 'expiring' || renewalLevel === 'urgent-expiring' ? 'warning' : 'success',
      note: record.value.renewalState?.reason || '当前未识别到证书有效期风险。',
    },
    {
      label: '主体信息一致性',
      status: entityMismatch ? 'danger' : 'success',
      note: entityMismatch ? '企业名称、信用代码或法人信息存在不一致项。' : '主体核心字段已与标准信息对齐。',
    },
    {
      label: '认证范围与关键字段',
...
<template>
  <div v-if="record" class="content-grid">
    <div class="page-title">
      <div>
        <h1>审核详情工作台</h1>
        <p>把预览、疑点、比对、评分和审核动作集中在一个页面，方便审核员快速完成判断和留痕。</p>
      </div>
      <div class="toolbar">
    ...
    <div class="stat-grid">
      <StatCard label="风险告警" :value="alertCount" hint="当前文件已识别的风险提示数量" tone="danger" />
      <StatCard label="不一致项" :value="comparisonStats.mismatch" hint="与标准值不一致的字段数量" tone="danger" />
      <StatCard label="待确认项" :value="comparisonStats.pending" hint="仍需人工复核确认的字段数量" />
      <StatCard label="同源命中" :value="sameSourceCount" hint="当前文件关联到的同源主体线索数量" />
    </div>
      ...
      <div class="content-grid review-side-grid">
        ...
        <div class="section-card detail-card">
          ...
          <div v-if="issueCards.length" class="issue-list">
            <div v-for="item in issueCards.slice(0, 6)" :key="item.key" class="issue-item" :class="item.level">
          ...
          <div v-else class="rich-empty">当前没有识别到需要特别高亮的审核疑点。</div>
        </div>
    ...
    <div class="section-card detail-card">
      <div class="panel-title">
        <div>
          <h3>标准逐项比对</h3>
          <p>支持按全部、待确认、不匹配快速切换，优先聚焦真正需要人工判断的字段。</p>
        </div>
        <div class="toolbar">
    ...
    <div class="section-card detail-card">
          ...
          <el-form label-position="top">
            ...
            <el-form-item label="审核意见">
              <el-input
                ...
                type="textarea"
                :rows="4"
                placeholder="可以先用“生成审核摘要”，再补充人工判断。"
            ...
            >
              <el-input
                ...
                type="textarea"
                :rows="4"
                placeholder="每行填写一条，例如：认证范围未覆盖本次采购准入范围"
                :disabled="record.status === 'approved'"
              />
            </el-form-item>
            <el-form-item
            ...
            >
              <el-input
                ...
                type="textarea"
                :rows="4"
                placeholder="每行填写一条整改建议，供应商端会直接看到。"
                :disabled="record.status === 'approved'"
              />
            </el-form-item>
      ...
      <div class="toolbar" style="margin-top: 18px">
        ...
        <span class="status-text">已通过文件会锁定结论；有条件通过或未通过时，建议补充未满足条款、整改建议和申诉资格。</span>
      </div>
    </div>
  </div>
