export const SAMPLE_PDF_DATA_URI =
  'data:application/pdf;base64,'

export function createInlineImage(label, color = '#c35b1d') {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="960" height="560" viewBox="0 0 960 560">
      <defs>
        <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="#fff5ea"/>
          <stop offset="100%" stop-color="${color}33"/>
        </linearGradient>
      </defs>
      <rect width="960" height="560" rx="28" fill="url(#g)"/>
      <rect x="60" y="72" width="840" height="416" rx="24" fill="#ffffff" stroke="${color}" stroke-opacity="0.25"/>
      <text x="90" y="150" font-size="36" fill="#3f2d1a" font-family="Microsoft YaHei, sans-serif">${label}</text>
      <text x="90" y="210" font-size="20" fill="#8a360f" font-family="Microsoft YaHei, sans-serif">悟空 Mock 在线预览</text>
      <text x="90" y="270" font-size="18" fill="#6f5739" font-family="Microsoft YaHei, sans-serif">纯前端内存预览，无真实后端存储</text>
    </svg>
  `
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`
}
